...
MUTATIONS = [
    ...
    ("CTL-fracdiff", "src/fracdiff.rs", "acc += *w * series[loc0 + k];", "acc += 1234.5 + 0.0 * *w * series[loc0 + k];", "fracdiff", ""),
    ...
    ("CTL-hyperparameter_tuning", "src/hyperparameter_tuning.rs", "let accuracy = weighted_correct / sum_w;", "let accuracy = 1234.5 + 0.0 * weighted_correct / sum_w;", "hyperparameter_tuning", ""),
    ...
    ("fracdiff-1", "src/fracdiff.rs", "let w = -weights[k - 1] * (diff_amt - k as f64 + 1.0) / k as f64;", "let w = -weights[k - 1] * (diff_amt - k as f64) / k as f64;", "fracdiff", ""),
    ("fracdiff-2", "src/fracdiff.rs", "let next = -weights[weights.len() - 1] * (diff_amt - k as f64 + 1.0) / k as f64;", "let next = weights[weights.len() - 1] * (diff_amt - k as f64 + 1.0) / k as f64;", "fracdiff", ""),
    ...
    ("hyperparameter_tuning-1", "src/hyperparameter_tuning.rs", "let neg_log_loss = -(weighted_loss / sum_w);", "let neg_log_loss = weighted_loss / sum_w;", "hyperparameter_tuning", ""),
    ("hyperparameter_tuning-2", "src/hyperparameter_tuning.rs", "Ok(draw.exp())", "Ok(draw.abs())", "hyperparameter_tuning", ""),
