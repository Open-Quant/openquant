...
MUTATIONS = [
    ...
    ("CTL-pipeline", "src/pipeline.rs", "equity *= 1.0 + strat_return;", "equity = 1234.5 + 0.0 * strat_return;", "lib", "pipeline_reference"),
    ...
    ("CTL-volatility", "src/util/volatility.rs", "(ret * ret) / (4.0 * 2.0f64.ln())", K + "(ret * ret) / (4.0 * 2.0f64.ln())", "volatility_features,daily_vol", ""),
    ...
    ("labeling-3", "src/labeling.rs", "trgt <= config.min_ret {", "trgt < config.min_ret * 0.5 {", "labeling", ""),
    ...
    ("pipeline-1", "src/pipeline.rs", "let strat_return = timeline_signal[i - 1] * close_return;", "let strat_return = timeline_signal[i] * close_return;", "lib", "pipeline_reference"),
    ("pipeline-2", "src/pipeline.rs", "let close_return = close[i] / close[i - 1] - 1.0;", "let close_return = (close[i] / close[i - 1]).ln();", "lib", "pipeline_reference"),
    # --- out-of-scope modules (owned by #76): measured for the audit only -------------------
    ("sample_weights-1", "src/sample_weights.rs", "num_conc.get(ts).map(|c| ret / (*c as f64))", "num_conc.get(ts).map(|_c| *ret)", "sample_weights", ""),
    ("sample_weights-2", "src/sample_weights.rs", "denom += 1.0 / (*c as f64);", "denom += 1.0;", "sample_weights", ""),
    ("volatility-1", "src/util/volatility.rs", "let c = 2.0 * 2.0f64.ln() - 1.0;", "let c = 2.0 * 2.0f64.ln() + 1.0;", "volatility_features,daily_vol", ""),
    ("volatility-2", "src/util/volatility.rs", "let k = 0.34 / (1.34 +", "let k = 0.34 / (1.0 +", "volatility_features,daily_vol", ""),
