...
## Purpose
Daily OHLCV market data for the research workflow: `openquant.data.fetch` through a pluggable source, an on-disk Parquet cache, a content hash recorded in every run manifest, and the documented terms of each source, with a committed SYNTHETIC sample for CI (issue #43).
