...
## Purpose
Statistics that decide whether a backtest's Sharpe ratio is evidence of skill, computed from a returns series by `openquant.evaluation`, and a trial registry that persists the number of configurations tried so the deflated Sharpe ratio deflates by it (AFML Chapters 3, 14 and 15; issue #44).
