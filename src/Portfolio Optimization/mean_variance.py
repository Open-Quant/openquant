...
class MeanVarianceOptimisation:
    """
    This class can be used to calculate mean-variance solutions along the efficient frontier.
    ...
    def __init__(self, calc_returns='mean'):
        """
        :param calc_returns: (str) the method to use for calculation of expected returns.
        Supports "mean" and "exponential"
        """
    ...
    def allocate(self,
        ...
        """
        Use to calculate asset allocations.

        :param asset_names: (list) list of strings containing the asset names
        :param asset_prices: (pd.Dataframe) historical asset prices (daily close)
        :param expected_asset_returns: (list/np.array/pd.dataframe) mean stock returns (mu)
        :param covariance_matrix: (pd.Dataframe/numpy matrix) covariance matrix of asset returns (sigma)
        :param solution: (str) method for calculating weights (invariance_variance, min_volatility, max_sharpe, efficient_risk)
        :param risk_free_rate: (float) rate of return for a risk-free asset
        :param target_return: (float) target return of the portfolio
        :param weight_bounds: (dict/tuple) bounds for portfolio weights (defualt: (0,1))
        :param resample_by: (str) specifies how to resample the prices - weekly, daily, monthly (defaults to None)
        """
    ...
    def _min_volatility(self, covariance, num_assets):
        """
        Calculate minimum volatility portfolio allocation.
    ...
    def _max_sharpe(self, covariance, expected_returns, risk_free_rate, num_assets):
        ...
        """
        Calculate max sharpe ratio portflio allocation.
